  ...
  return (
    <div className="w-full h-[100dvh] p-6 bg-[#f8f9fa] font-['Pretendard_GOV',sans-serif]">
      <div className="flex items-center gap-2 mb-6">
        <button className="bg-transparent border-0 p-0 cursor-pointer" onClick={() => navigate('/schedule')}>
          <img src={left_chevron} alt="뒤로가기" className="w-6 h-6" />
        </button>
        <h1 className="text-xl font-semibold text-[#212121]">근로 시간 조회</h1>
      </div>

      <div className="inline-block bg-white border border-[#dee2e6] rounded-full py-2 px-4 text-sm font-medium text-[#495057] mb-4 shadow-[0_1px_2px_rgba(0,0,0,0.04)]">2025년 9월</div>
      ...
      <div className="flex flex-col gap-3">
        {weeklyRecords.map((record, idx) => (
          <div key={idx} className="bg-white rounded-xl p-4 shadow-[0_2px_6px_rgba(0,0,0,0.03)] flex justify-between items-center">
