...
const allSlotsValid = (slots: string[]) =>
  slots.length > 0 && slots.every(isSlotWithinMorning);
...
async function fetchMonthlyApplies(monthISO: string): Promise<FetchResult> {
  ...
  return new Promise((res) =>
    setTimeout(() => res({ month: monthISO, items: demo }), 150)
  );
}
              ...
              {rows.map((it) => {
                ...
                return (
                  <article
                    key={it.id}
                    className={
                      "apply-card" + (valid ? "" : " apply-card--warn")
                    }
                  >
                    <div className="apply-head">
                      <div className="apply-name">{it.name}</div>
                      <span
                        className={`status status--${it.status.toLowerCase()}`}
                      >
                        {it.status === "PENDING"
                          ? "대기"
                          : it.status === "APPROVED"
                    ...
                    <div className="apply-slots">
                      {it.slots.map((s) => (
                        <span
                          ...
                          className={
                            "slot" +
                            (isSlotWithinMorning(s) ? "" : " slot--error")
                          }
                        >
                          {s}
                        </span>
