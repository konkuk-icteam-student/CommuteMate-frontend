...
export const validateWeeklyHours = (
  ...
  const errors = isValid ? [] : [`주간 근무시간이 ${MAX_WEEKLY_HOURS}시간을 초과합니다 (현재: ${totalHours}시간)`];
  const warnings = totalHours >= MAX_WEEKLY_HOURS * 0.9 && isValid
...
export const validateMonthlyHours = (
  ...
  const errors = isValid ? [] : [`월간 근무시간이 ${MAX_MONTHLY_HOURS}시간을 초과합니다 (현재: ${totalHours}시간)`];
  const warnings = totalHours >= MAX_MONTHLY_HOURS * 0.9 && isValid
...
export const validateTimeSlotCapacity = (currentWorkers: number): ValidationResult => {
  ...
  const errors = isValid ? [] : [`해당 시간대는 정원(${MAX_WORKERS_PER_TIMESLOT}명)이 마감되었습니다`];
  const warnings = currentWorkers >= MAX_WORKERS_PER_TIMESLOT - 1 && isValid
...
export const validateAttendanceApplication = (
  ...
  const isValid = weeklyValidation.isValid && monthlyValidation.isValid && capacityValidation.isValid;
  const errors = [
